    ...
    function createRequest () {
        ...
        if (!value_exchangeRequestId && !value_exchangeRequestHash) {
            const
                ...
                cryptoAddress = addressWallet,
                networkId = network.id,
                partnerName = partner,
                exchangeRequestUserAgentInfo = {
            ...
            params = { currencyInId, currencyOutId, amountIn, amountOut, cryptoAddress, networkId, partnerName, exchangeRequestUserAgentInfo }
            setLoading(true)
            setDisableButton(true)
            requests.Exchange(params).then(response =>{
                setLoading(false)
                setDisableButton(false)
                if (response &&response.exchangeRequestId && response.hash && response.tradeUserId) {
